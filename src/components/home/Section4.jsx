...
const FILE_BASE_URL = import.meta.env.VITE_API_URL?.replace("/api/v1", "") || "http://localhost:8080";
...
const Section4 = () => {
    ...
    const loadEvents = async () => {
      try {
        ...
        const processedEvents = data.response.map(event => ({
          ...event,
          image: event.image
            ? `${FILE_BASE_URL}${event.image.startsWith("/") ? event.image : "/" + event.image}`
            : "",
        }));
  ...
  const getImageUrl = (imagePath) => {
    if (!imagePath) return 'https://via.placeholder.com/800x450/e5e7eb/6b7280?text=Aucune+image';
    return imagePath;
  };
                  ...
                  <div className="flex-1 relative aspect-video bg-gray-200">
                    <img
                      ...
                      onError={(e) => { 
                        ...
                        e.target.src = 'https://via.placeholder.com/800x450/e5e7eb/6b7280?text=Image+indisponible';
                      }}
                    />
