		...
		http.csrf(AbstractHttpConfigurer::disable).cors(cors -> cors.configurationSource(corsConfigurationSource))
				...
				.authorizeHttpRequests(auth -> auth
						...
						.requestMatchers("/api/v1/events/public", "/api/v1/events/public/**", "/api/v1/auth/**",
								...
								"/swagger-ui.html", "/api/v1/contact", "/api/v1/contact/**")
						.permitAll()

						// Endpoints nécessitant une authentification (rôles non implémentés)
						.requestMatchers(HttpMethod.POST, "/api/v1/events", "/api/v1/events/**").authenticated()
						.requestMatchers(HttpMethod.PUT, "/api/v1/events/**").authenticated()
						.requestMatchers(HttpMethod.DELETE, "/api/v1/events/**").authenticated()
						.requestMatchers("/api/v1/files/upload", "/uploads/**").authenticated()

						// Tous les autres endpoints nécessitent une authentification / vérification
						// d'email
						.anyRequest().access(emailVerificationAuthorizationManager));
