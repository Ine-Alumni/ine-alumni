...
public class LaureatController {
	...
	@Operation(summary = "Get available positions for filtering", description = "Get list of all distinct current positions from verified alumni")
	public ResponseEntity<ApiResponseDto<List<String>>> getAvailablePositions() {
		List<String> positions = laureatService.getAvailablePositions();
		ApiResponseDto<List<String>> response = ApiResponseDto.<List<String>>builder().isSuccess(true)
				.message("Available positions retrieved").response(positions).build();
		return ResponseEntity.ok(response);
	}

	@GetMapping("/filter-options/cities")
	@Operation(summary = "Get available cities for filtering", description = "Get list of all distinct cities from verified alumni")
	public ResponseEntity<ApiResponseDto<List<String>>> getAvailableCities() {
		List<String> cities = laureatService.getAvailableCities();
		ApiResponseDto<List<String>> response = ApiResponseDto.<List<String>>builder().isSuccess(true)
				.message("Available cities retrieved").response(cities).build();
		return ResponseEntity.ok(response);
	}
}
