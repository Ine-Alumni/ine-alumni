  ...
  const { values, errors, touched, isSubmitting, handleChange, handleSubmit } =
    useFormik({
      ...
      onSubmit: async (vals, actions) => {
        try {
          const res = await forgotPassword(vals.email);
          if (res.status < 400) {
            addAlert(true, res.data || "A reset code has been sent to your email address.");
            const email = vals.email;
            actions.resetForm();
            navigate("/reset-password", { state: { email } });
          } else {
            addAlert(false, res.data || "Unable to process request. Please try again.");
          }
        } catch (e) {
